...
When face swap is on: `cam_fx_try_get_frame` → if a fresh swapped frame exists: upload it to a persistent `frame_tex` (recreate on size change; `gs_texture_set_image` or recreate), then run the EXISTING composite path but with `image` = frame_tex: the DrawTransparent/DrawReplace/DrawBlur techniques sample `image` — for transparent/replace, set the `image` param explicitly to frame_tex (the process_filter_begin auto-bound texture is the unswapped source — override after begin). For blur: the Kawase helper blurs frame_tex instead of re-rendering the target. The mask comes from `cam_fx_try_get_mask` as usual (the pipeline computes it from the swapped frame).
